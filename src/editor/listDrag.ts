...
interface ListDragState {
  ...
  dragging: number | null; // position before the dragged list_item
...
function buildHandleDecos(doc: PMNode): Decoration[] {
...
function renderHandle(view: EditorView, getPos: () => number | undefined) {
  ...
  dom.addEventListener("dragstart", (e) => {
    ...
    const itemPos = widgetPos - 1; // position before the list_item node
    ...
    view.dispatch(view.state.tr.setMeta(listDragKey, { type: "start", pos: itemPos }));
  });

  dom.addEventListener("dragend", () => {
    if (listDragKey.getState(view.state)?.dragging != null) {
...
export function buildListDragPlugin(): Plugin<ListDragState> {
  return new Plugin<ListDragState>({
    ...
    state: {
      ...
      apply(tr, prev) {
        let { handles, dragging, dropTarget } = prev;
        if (tr.docChanged) {
        ...
        const meta = tr.getMeta(listDragKey) as
          ...
          | { type: "over"; target: DropTarget | null }
          | { type: "end" }
          | undefined;
        if (meta) {
          if (meta.type === "start") {
        ...
        return { handles, dragging, dropTarget };
      },
    },
    props: {
      decorations(state) {
        ...
        if (ps.dragging != null) {
          const node = state.doc.nodeAt(ps.dragging);
          if (node) {
            extra.push(
              Decoration.node(ps.dragging, ps.dragging + node.nodeSize, {
      ...
      handleDOMEvents: {
        dragover(view, event) {
          ...
          if (!ps || ps.dragging == null) return false;
          const target = dropTargetFromEvent(view, event, ps.dragging);
          if (target) {
            event.preventDefault();
            if (event.dataTransfer) event.dataTransfer.dropEffect = "move";
          }
          if (!sameTarget(ps.dropTarget, target)) {
            view.dispatch(view.state.tr.setMeta(listDragKey, { type: "over", target }));
          }
          return false;
        },
        drop(view, event) {
          ...
          if (!ps || ps.dragging == null) return false;
          event.preventDefault();
          const target = ps.dropTarget ?? dropTargetFromEvent(view, event, ps.dragging);
          if (target) {
            const tr = moveListItem(view.state, ps.dragging, target);
            if (tr) {
